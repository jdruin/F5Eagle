...
SET GET_ISNETFX2_CMD=EagleShell.exe -eval "puts stdout [expr {[string match 2.* [info runtimeversion]] ? {True} : {False}}]"
...
SET GET_MAJORMINOR_CMD=EagleShell.exe -eval "puts stdout [join [lrange [split %PATCHLEVEL% .] 0 1] .]"
...
SET GET_ASSEMBLY_CMD=EagleShell.exe -eval "puts stdout [lindex [info assembly] 0]"
...
SET GET_COMPILEINFO_CMD=EagleShell.exe -eval "puts stdout [getCompileInfo]"
