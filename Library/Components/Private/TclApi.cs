...
namespace Eagle._Components.Private.Tcl
{
    ...
    sealed class TclApi : ITclApi, IDisposable, ICloneable
    {
        ...
        internal struct NativeStubs
        {
            ...
            public IntPtr setSlaveCancelFlags;
        ...
        //       in bytes.
        //
        private static readonly int INTERP_ERRORLINE_OFFSET;

        //
        // NOTE: The offset into the Tcl_Interp structure for the numLevels member,
        //       in bytes.
        //
        internal static readonly int INTERP_NUMLEVELS_OFFSET;
        #endregion
            ...
            )
        {
            lock (syncRoot)
            {
                ...
                addresses.Add(typeof(TclSetSlaveCancelFlags), IntPtr.Zero); /* TIP #285 */
            ...
            )
        {
            ...
            addresses[typeof(TclSetSlaveCancelFlags)] = nativeStubs.setSlaveCancelFlags; /* TIP #285 */
            ...
            )
        {
            lock (syncRoot)
            {
                ...
                delegates.Add(typeof(TclSetSlaveCancelFlags), null); /* TIP #285 */
                ...
                optional.Add(typeof(TclSetSlaveCancelFlags), true); /* OPTIONAL: TIP #285 */
            ...
            )
        {
            ...
            if (interp != IntPtr.Zero)
            {
                ...
                if (interpreter != null)
                {
                    int currentThreadId = GlobalState.GetCurrentNativeThreadId();
                    int threadId = 0;
...
#if !TCL_THREADS
        private int GetThreadIdForObjPtr(
            IntPtr objPtr
            )
        {
            //
            ...
            )
        {
            ...
            if (objPtr != IntPtr.Zero)
            {
                ...
                //
                int currentThreadId = GlobalState.GetCurrentNativeThreadId();
                int threadId = GetThreadIdForObjPtr(objPtr);
        ...
        public TclSetSlaveCancelFlags SetSlaveCancelFlags
        {
            get
            {
                CheckDisposed();
                ...
                {
                    return (delegates != null) ?
                        (TclSetSlaveCancelFlags)delegates[typeof(TclSetSlaveCancelFlags)] : null;
                }
            }
        }
