...
namespace Eagle._Components.Private
{
    ...
    internal static class ConversionOps
    {
        ...
        {
            //
            // NOTE: Determine if the specified delegate type is the System.Delegate
            //       type itself (this requires some special handling).
            //
            ...
            if (IsAsyncCallback(type) ||
                IsEventHandler(type) ||
                IsThreadStart(type) ||
                IsParameterizedThreadStart(type) ||
                IsDynamicInvokeCallback(type) ||
                (useDelegateCallback && isDelegate) ||
                (useGenericCallback && IsGenericCallback(type)))
            {
                return true;
            }
        ...
        public static bool IsAsyncCallback(Type type)
        {
            if (type == typeof(AsyncCallback))
                return true;
            ...
            if (IsDelegateType(type, false))
            {
                ...
                {
                    //
                    // NOTE: We need an instance of the AsyncCallback delegate so that we can
                    ...
                    //
                    // NOTE: Attempt to create a delegate with a method signature compatible
                    //       with the AsyncCallback delegate method from the specified type.
                    //
        ...
        public static bool IsEventHandler(Type type)
        {
            ...
            if (IsDelegateType(type, false))
            {
                ...
                {
                    //
                    // NOTE: We need an instance of the EventHandler delegate so that we
                    //       can get the method signature (i.e. MethodInfo) for it.
                    //
                    DelegateMethods delegateMethods = new DelegateMethods();
                    ...
                    //
                    // NOTE: Attempt to create a delegate with a method signature compatible
                    //       with the EventHandler delegate method from the specified type.
                    //
                    Delegate @delegate = Delegate.CreateDelegate(type, null,
        ...
        public static bool IsThreadStart(Type type)
        {
            ...
            if (IsDelegateType(type, false))
            {
                ...
                {
                    //
                    // NOTE: We need an instance of the ThreadStart delegate so that we
                    //       can get the method signature (i.e. MethodInfo) for it.
                    //
                    DelegateMethods delegateMethods = new DelegateMethods();
                    ...
                    //
                    // NOTE: Attempt to create a delegate with a method signature compatible
                    //       with the ThreadStart delegate method from the specified type.
                    //
                    Delegate @delegate = Delegate.CreateDelegate(type, null,
        ...
        {
            if (type == typeof(ParameterizedThreadStart))
                return true;
            ...
            if (IsDelegateType(type, false))
            {
                ...
                {
                    //
                    // NOTE: We need an instance of the ParameterizedThreadStart delegate so
                    //       that we can get the method signature (i.e. MethodInfo) for it.
                    //
                    DelegateMethods delegateMethods = new DelegateMethods();
                    ...
                    //
                    // NOTE: Attempt to create a delegate with a method signature compatible
                    //       with the ParameterizedThreadStart delegate method from the
                    //       specified type.
                    //
                    Delegate @delegate = Delegate.CreateDelegate(type, null,
        ...
        public static bool IsGenericCallback(Type type)
        {
            if (type == typeof(GenericCallback))
                return true;
            ...
            if (IsDelegateType(type, false))
            {
                ...
                {
                    //
                    // NOTE: We need an instance of the GenericCallback delegate so that we
                    //       can get the method signature (i.e. MethodInfo) for it.
                    //
                    DelegateMethods delegateMethods = new DelegateMethods();
                    ...
                    //
                    // NOTE: Attempt to create a delegate with a method signature compatible
                    //       with the GenericCallback delegate method from the specified type.
                    //
                    Delegate @delegate = Delegate.CreateDelegate(type, null,
        ...
        public static bool IsDynamicInvokeCallback(Type type)
        {
            if (type == typeof(DynamicInvokeCallback))
                return true;
            ...
            if (IsDelegateType(type, false))
            {
                ...
                {
                    //
                    // NOTE: We need an instance of the DynamicInvokeCallback delegate so
                    //       that we can get the method signature (i.e. MethodInfo) for it.
                    //
                    DelegateMethods delegateMethods = new DelegateMethods();
                    DynamicInvokeCallback dynamicInvokeCallback = new DynamicInvokeCallback(
                        delegateMethods.NullDynamicInvokeCallback);

                    //
                    // NOTE: Attempt to create a delegate with a method signature
                    //       compatible with the DynamicInvokeCallback delegate
                    //       method from the specified type.
                    //
                    Delegate @delegate = Delegate.CreateDelegate(type, null,
        ...
        #region Dead Code
#if DEAD_CODE
        private static long ToLong(double X) /* SAFE */
        {
                    ...
                    )
                {
                    ...
                    else if (value is DateTime)
                    {
                        ...
                        if (dateTimeFormat != null)
                            text = dateTime.ToString(dateTimeFormat, cultureInfo);
                        else
            ...
            internal static class ChangeType
            {
                    ...
                    )
                {
                    ...
                    else
                    {
                        ...
                        if (Value.GetSignedByte2(
                                text, ValueFlags.AnyByte | ValueFlags.Signed,
                    ...
                    )
                {
                    ...
                    else
                    {
                        ...
                        if (Value.GetNarrowInteger2(
                                text, ValueFlags.AnyNarrowInteger, cultureInfo,
                                ref shortValue, ref error) == ReturnCode.Ok)
                        {
                            value = shortValue;
                            return ReturnCode.Ok;
                        }
                    ...
                    )
                {
                    ...
                    else
                    {
                        ...
                        if (Value.GetInteger2(
                                text, ValueFlags.AnyInteger, cultureInfo,
                    ...
                    )
                {
                    if (type != null)
                    {
                        ...
                        if (MarshalOps.IsEnumType(type, true, true, ref elementType))
                        {
                            ...
                            if (EnumOps.IsFlagsEnum(elementType))
                            {
                    ...
                    )
                {
                    ...
                    else
                    {
                        ...
                        ObjectOps.ProcessDateTimeOptions(
                            interpreter, options, null, null, out dateTimeKind,
                            out dateTimeFormat);
                        ...
                        if (Value.GetDateTime2(
                                ...
                                dateTimeKind, cultureInfo, ref dateTime,
                                ref error) == ReturnCode.Ok)
                        {
                            value = dateTime;
                            return ReturnCode.Ok;
                    ...
                    )
                {
                    ...
                    else
                    {
                        ...
                        if (Value.GetDecimal(
                                text, cultureInfo, ref decimalValue,
                                ref error) == ReturnCode.Ok)
                        {
                            value = decimalValue;
                            return ReturnCode.Ok;
                    ...
                    )
                {
                    ...
                    if (Value.GetType(interpreter, text, null,
                    ...
                    )
                {
                    ...
                    if (Value.GetValue(
                            text, dateTimeFormat, ValueFlags.AnyStrict, dateTimeKind,
                            cultureInfo, ref localValue, ref error) == ReturnCode.Ok)
                    {
                        try
                        {
                            ...
                            {
                                value = Convert.ChangeType(localValue, type); /* throw */
                            }
                            else
                            {
                                value = localValue;
                                    ...
                                    @object, ref error))
                            {
                                ...
                                else
                                {
                                    error = String.Format(
                                        ...
                                        type.FullName, text, objectType.FullName);
                                }
                            }
                        }
                    }
                ...
                private static ReturnCode ToCommandCallback(
                    Interpreter interpreter,
                    Type type,
                    string text,
                    OptionDictionary options, /* NOT USED */
                    ...
                    if (type != null)
                    {
                        if (IsDelegateType(type, false))
                        {
                            ...
                            if (IsSupportedDelegateType(
                                    ...
                                    out isDelegate) || useDynamicCallback)
                            {
                                //
                                        ...
                                        ref list, ref error) == ReturnCode.Ok)
                                {
                                    ...
                                    //
                                    // NOTE: If this flag is set, ignore any options that
                                    //       may be set (they will still be parsed).
                                    //
                                    ...
                                    //
                                    bool throwOnBindFailure = FlagOps.HasFlags(
                                        marshalFlags, MarshalFlags.ThrowOnBindFailure,
                                        true);

                                    OptionDictionary localOptions = null;

                                    if (!noCallbackOptions)
                                        localOptions = ObjectOps.GetCallbackOptions();
                                            ...
                                            ref error) == ReturnCode.Ok))
                                    {
                                            ...
                                            (argumentIndex != Index.Invalid))
                                        {
                                            ...
                                            //
                                            // NOTE: If we succeed, make sure the callback
                                            //       has updated marshal flags preventing
                                            //       future (superfluous) errors.
                                            //
                            ...
                            else
                            {
                                error = ScriptOps.BadValue(
                                    "unsupported", "delegate type", type.FullName,
                                    new string[] {
                                        useDelegateCallback ?
