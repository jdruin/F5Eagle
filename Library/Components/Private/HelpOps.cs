...
namespace Eagle._Components.Private
{
    ...
    internal static class HelpOps
    {
        ...
        private const int HelpWidth = 79; /* magic */

        private const int GroupsPerLine = 6; /* magic */
        private const int TopicsPerLine = 6; /* magic */
        ...
        //
        private static readonly string HelpBeginMagic = "# <help>";
        private static readonly string HelpEndMagic = "# </help>";
#endif
#endif

        private const string certificateSubjectPrefix = " - ";

        private const string bannerFormat = "{0} v{1} {2}{3} {4} ({5}){6}";
...
#if SHELL && INTERACTIVE_COMMANDS
        public static int ClearHelpCache()
        {
            ...
            )
        {
            ...
            int beginIndex = text.IndexOf(
                HelpBeginMagic, StringOps.SystemStringComparisonType);

            if (beginIndex == Index.Invalid)
                return null;

            int beginMagicLength = HelpBeginMagic.Length;

            int endIndex = text.IndexOf(
                HelpEndMagic, beginIndex + beginMagicLength,
                StringOps.SystemStringComparisonType);

            if (endIndex == Index.Invalid)
                return null;

            int endMagicLength = HelpEndMagic.Length;
            ...
            code = XmlOps.GetHelp(document, ref value, ref error);
            ...
            )
        {
            ...
            return IsInteractiveExtension(identifierBase.Name);
        }
        ...
        private static string MaybeAdjustHelpItemTopic(
            string topic,     /* in */
            ref string prefix /* out */
            )
        {
            if (!String.IsNullOrEmpty(topic))
            {
                foreach (string localPrefix in new string[] {
                    ShellOps.InteractiveSystemCommandPrefix,
                    ShellOps.InteractiveCommandPrefix })
                {
                    ...
                    {
                        prefix = localPrefix;
                        return topic.Substring(localPrefix.Length);
                    }
                }
            }

            prefix = ShellOps.DefaultInteractiveCommandPrefix;
            return topic;
        }
            ...
            )
        {
            ...
            EngineFlags engineFlags = interpreter.GetResolveEngineFlags(true);
            ...
            )
        {
            ...
            if (groups != null)
            {
                ...
                if (groups.TryGetValue(topic, out helpGroup) &&
                    (helpGroup != null))
                {
                    if (helpGroup.Count > 0)
                    {
                        return String.Format(
                            ...
                            GenericOps<string>.ListToEnglish(
                                helpGroup, ", ", Characters.Space.ToString(),
                                "and ", ShellOps.InteractiveCommandPrefix,
                                null));
                    }
                    else
                    {
            ...
            if (help != null)
            {
                ...
                string localTopic = MaybeAdjustHelpItemTopic(
                    topic, ref prefix);
                ...
                if (help.TryGetValue(localTopic, out helpItem))
                {
                    if (helpItem != null)
                    {
                        if ((helpItem.X != null) && (helpItem.Y != null))
                        {
                            ...
                            else
                            {
                                return String.Format(
                                    ...
                                    prefix, localTopic, helpItem.X, helpItem.Y);
                            }
                        }
                        ...
                        else
                        {
                            ...
                            {
                                return String.Format(
                                    "No help is available for command {0}.",
                                    FormatOps.WrapOrNull(localTopic));
                            }
                        }
                    }
                    else
                    {
                        return String.Format(
                            "Invalid help item for command {0}.",
                            FormatOps.WrapOrNull(localTopic));
                    }
                }
                else
                {
                    ...
                    if (GetIExecuteViaResolvers(
                            ...
                            ref execute) == ReturnCode.Ok)
                    {
                        return FormatHelpItem(execute, name, summary);
                    }
                }
            }
            else
            ...
            //
            // NOTE: If no syntax help was available or the command was not
            //       found we simply return a suitable error message.
            //
            return String.Format(
                "There is no help topic {0}.", FormatOps.WrapOrNull(topic));
        }
        ...
        {
            StringPairDictionary help = GetCachedInteractiveCommandHelp(interpreter);

            if (help != null)
            {
                StringList result = new StringList(); /* COPY */

                ReturnCode code;
                Result error = null;

                code = GenericOps<string>.FilterList(
                    new StringList(help.Keys), result, 0, Index.Invalid,
            ...
            )
        {
            ...
            {
                return String.Format(
                    "{0}{1}command", EntityOps.IsHidden((ICommand)execute) ?
                    hiddenPrefix : String.Empty, IsInteractiveExtension(execute) ?
                    interactiveExtensionPrefix : String.Empty);
            }

            if (execute is IProcedure)
            {
                return String.Format(
                    "{0}{1}procedure", EntityOps.IsHidden((IProcedure)execute) ?
                    hiddenPrefix : String.Empty, IsInteractiveExtension(execute) ?
                    interactiveExtensionPrefix : String.Empty);
            }

            Type type = execute != null ? execute.GetType() : null;

            if (type != null)
            {
                string result = FormatOps.TypeNameOrFullName(type);

                if (!String.IsNullOrEmpty(result))
                {
                    return String.Format(
                        "{0}{1}", IsInteractiveExtension(execute, name) ?
                        interactiveExtensionPrefix : String.Empty, result);
                }
            }
        ...
        {
            string result = GetSyntax(command, null);
            ...
            )
        {
            ...
            if (syntax != null)
                return GetSyntax(syntax, @default);
            ...
            )
        {
            ...
            if (command != null)
                return GetSyntaxForCommand(command, @default);

            IProcedure procedure = execute as IProcedure;

            if (procedure != null)
                return GetSyntaxForProcedure(procedure, @default);

            ISyntax syntax = execute as ISyntax;
                ...
                if (localNames[0] != null)
                {
                    foreach (string name in localNames[0])
                    {
                        ...
                        if (getHelp)
                        {
                            ...
                            help[newName] = new StringPair(GetSyntaxForCommand(
                                command, null), GetDescriptionForCommand(command,
                                    "Interactive extension command."));
                        }
                    }
                }
                ...
                if (localNames[1] != null)
                {
                    foreach (string name in localNames[1])
                    {
                        ...
                        if (getHelp)
                        {
                            ...
                            help[newName] = new StringPair(GetSyntaxForProcedure(
                                procedure, null), GetDescriptionForProcedure(procedure,
                ...
                if (localNames[2] != null)
                {
                    foreach (string name in localNames[2])
                    {
                        ...
                        if (getHelp)
                        {
                            ...
                            help[newName] = new StringPair(GetSyntaxForIExecute(
                                execute, null), GetDescriptionForIExecute(execute,
                ...
                if (localNames[3] != null)
                {
                    foreach (string name in localNames[3])
                    {
                        ...
                        if (getHelp)
                        {
                            ...
                            help[newName] = new StringPair(GetSyntaxForCommand(
                                command, null), GetDescriptionForCommand(command,
                                    "Interactive extension command (hidden)."));
                        }
                    }
                }
        ...
        private static StringPairDictionary GetInteractiveCommandHelp() /* CANNOT RETURN NULL */
        {
            ...
            result.Add("check",
                new StringPair("?wantScripts? ?quiet? ?prompt? ?automatic? ?actionType? ?releaseType?", String.Format(
            ...
            result.Add("clearr",
                new StringPair(null,
                    "Clears the local result.  The master result is untouched."));
            ...
            result.Add("copyr",
                new StringPair(null,
                    "Copies the master result into the local result.  The master result is untouched."));
            ...
            result.Add("fresr",
                new StringPair(null,
                    "Resets the master and local results in-place."));
            ...
            result.Add("help",
                new StringPair("?topic? ?groups? ?commands? ?header?",
                    "Displays information on the specified help topic or a list of available help topics."));
            ...
            result.Add("lrinfo",
                new StringPair(null,
                    ...
                    "The master result is untouched."));
            ...
            result.Add("mover",
                new StringPair(null,
                    "Moves the local result into the master result.  The local result is reset."));

            /* MASTER RESULT */
            result.Add("mrinfo",
                new StringPair("?previous?",
                    "Displays detailed information about the master result, leaving it untouched.  " +
            ...
            result.Add("nextr",
                new StringPair(null,
                    "Sets the previous result for the interactive interpreter to the local result.  The master result is untouched."));
#endif
            ...
            result.Add("nop",
                new StringPair(null,
                    "Does nothing.  The local and master results are untouched."));
            ...
            result.Add("nullr",
                new StringPair(null,
                    "Sets the local result to null.  The master result is untouched."));
            ...
            result.Add("overr",
                new StringPair("?options?",
                    "Sets the local result to the specified value.  The master result is untouched."));
            ...
            result.Add("prevr",
                new StringPair(null,
                    "Sets the local result to the previous result for the interactive interpreter.  The master result is untouched."));
#endif
            ...
            result.Add("resc",
                new StringPair(null,
                    "Resets the cancel and unwind flags for the interactive interpreter."));
            ...
            result.Add("resh",
                new StringPair(null,
                    "Resets the halt flag for the interactive interpreter."));

            /* LOCAL/MASTER RESULT */
            result.Add("resr",
                new StringPair(null,
                    "Resets the master and local results."));
            ...
            result.Add("restt",
                new StringPair("?strict?",
                    "Restores the trace plugin for the interactive interpreter."));
#endif
            ...
            result.Add("rinfo",
                new StringPair(null,
                    "Displays the master and local results, leaving them untouched."));
            ...
            result.Add("setr",
                new StringPair(null,
                    "Copies the local result into the master result.  The local result is untouched."));
            ...
            /* LOCAL/MASTER RESULT */
            result.Add("sresult",
                new StringPair("?varName? ?master?",
                    "Stores the local or master result into the specified variable.  The local and master results are untouched."));
            ...
            result.Add("tclshrc",
                new StringPair("?arg ...?", String.Format(
                    ...
                    FormatOps.WrapOrNull(TclVars.RunCommandsFileName))));
            ...
            result.Add("usage",
                new StringPair("?banner? ?legalese? ?options? ?environment?",
            ...
            result.Add("version",
                new StringPair("?banner? ?legalese? ?source? ?update? ?plugins? ?options?",
            ...
            )
        {
            ...
            if (writeHost != null)
            {
                ...
                {
                    if (FlagOps.HasFlags(
                            writeHost.GetHostFlags(), HostFlags.AllColors, false))
                    {
                        if (newLine)
                        {
            ...
            )
        {
            ...
            string text = SharedAttributeOps.GetAssemblyText(assembly);

            return String.Format(
                bannerFormat, GlobalState.GetPackageName(),
                ...
                RuntimeOps.GetCertificateSubject(fileName,
                    certificateSubjectPrefix, true, true));
        }
        ...
        {
            return String.Format(
                Vars.PackageDescription, TclVars.VersionValue);
        }
            ...
            )
        {
            if (interpreter != null)
            {
                ...
                if (displayHost != null)
                {
                    ...
                    string value = GetVersionLine(assembly, fileName);
                    bool wrote = false;
                    ...
                    if (!noOfficial)
                    {
                        ...
                        value = official ?
                            Vars.OfficialDescription : Vars.UnofficialDescription;
                    ...
                    if (!noTrusted)
                    {
                        ...
                        value = trusted ?
                            Vars.TrustedDescription : Vars.UntrustedDescription;

                        if (!String.IsNullOrEmpty(value))
                        {
                            if (wrote && noOfficial && !displayHost.WriteLine())
                                return false;
                    ...
                    if (!noStable)
                    {
                        ...
                        value = stable ?
                            Vars.StableDescription : Vars.UnstableDescription;

                        if (!String.IsNullOrEmpty(value))
                        {
                            if (wrote && noOfficial && noTrusted && !displayHost.WriteLine())
                                return false;
                    ...
                    if (!noSafe)
                    {
                        ...
                        value = String.Format(
                            Vars.SafeDescription, FormatOps.InterpreterNoThrow(interpreter),
                            safe ? "safe" : "unsafe");

                        if (!String.IsNullOrEmpty(value))
                        {
                            if (wrote && noOfficial && noTrusted && noStable &&
                    ...
                    if (!noSecurity)
                    {
                        ...
                        value = String.Format(
                            Vars.SecurityDescription, FormatOps.InterpreterNoThrow(interpreter),
                            security ? "enabled" : "disabled");

                        if (!String.IsNullOrEmpty(value))
                        {
                            if (wrote && noOfficial && noTrusted && noStable && noSafe &&
                                !displayHost.WriteLine())
                            {
                                return false;
                            }

                            if (wrote && noSafe && !displayHost.WriteLine())
                                return false;
                    ...
                    if (!noPlugins)
                    {
                        ...
                        value = String.Format(
                            Vars.IsolatedDescription, FormatOps.InterpreterNoThrow(interpreter),
                            isolated ? "enabled" : "disabled");

                        if (!String.IsNullOrEmpty(value))
                        {
                            if (wrote && noOfficial && noTrusted && noStable && noSafe &&
                                noSecurity && !displayHost.WriteLine())
                            {
                                return false;
                            }

                            if (wrote && noSafe && noSecurity && !displayHost.WriteLine())
                                return false;
            ...
            try
            {
                if (interpreter != null)
                {
                    ...
                    if (displayHost != null)
                    {
                        ...
                        string executableFileName = Path.GetFileName(PathOps.GetExecutableName());
                        ...
                        if (showOptions)
                        {
                            ...
                            displayHost.WriteLine(String.Format(
                                ...
                                Characters.HorizontalTab, CommandLineOption.Namespaces, TclVars.VersionValue));
                            displayHost.WriteLine();
#if CONFIGURATION
                            displayHost.WriteLine(String.Format(
                                "{0}-{1} : This option is processed prior to the standard\n" +
                            ...
                            displayHost.WriteLine(String.Format(
                                ...
                                "{0}                     using the Harpy and Badge security plugins, and\n" +
                                "{0}                     then continues processing arguments.  If any of\n" +
                                "{0}                     the necessary plugins are unavailable, an error\n" +
                                "{0}                     is generated.",
                        ...
                        if (showEnvironment)
                        {
                            ...
                            displayHost.WriteLine(String.Format(
                                ...
                                "{0}default cache level will be artificially increased by one.",
                            ...
                            displayHost.WriteLine(String.Format(
                                "{0}If the \"{1}\" or \"{2}\" environment variables are set\n" +
                                "{0}[to anything], their values will be used by the test suite as the name\n" +
                                "{0}of the directory where temporary files should be stored.",
                            ...
                            displayHost.WriteLine(String.Format(
                                ...
                                "{0}the Harpy and Badge security plugins from being loaded.",
                            ...
                            displayHost.WriteLine(String.Format(
                                ...
                                Characters.HorizontalTab, EnvVars.NoUtility));
                            displayHost.WriteLine();
                            displayHost.WriteLine(String.Format(
        ...
        private static ReturnCode GetPluginAbout(
            Interpreter interpreter, /* in */
            IPlugin plugin,          /* in */
            ref Result result        /* out */
            )
        {
            try
            {
                return plugin.About(interpreter, ref result); /* throw */
            }
            catch (Exception e)
            {
                TraceOps.DebugTrace(
            ...
            )
        {
            if (!wrote)
            {
                ...
                if (system)
                {
                    ...
                    //
                    if (showBanner || showLegalese || showSource || showUpdate)
                        interactiveHost.WriteLine();
                }
                else
                {
                    ...
                    //
                    if (wrotePlugin ||
                        showBanner || showLegalese || showSource || showUpdate)
                    {
                        interactiveHost.WriteLine();
                    }
                }
                ...
                interactiveHost.WriteLine(String.Format(
                    "Using {0} plugins:", system ? "core" : "loaded"));
            ...
            )
        {
            ...
            if (aboutList != null)
            {
                MaybeWritePluginsHeader(
                    displayHost, system, showBanner, showLegalese, showSource,
                    showUpdate, wrotePlugin, ref wrote);
                    ...
                    if (displayHost != null)
                    {
                        ...
                        if (showPlugins)
                        {
                            ...
                            if (plugins != null)
                            {
                                ...
                                foreach (bool system in new bool[] { true, false })
                                {
                                    ...
                                    foreach (KeyValuePair<string, _Wrappers.Plugin> pair in plugins)
                                    {
                                        ...
                                        try
                                        {
                                            pluginFlags = plugin.Flags; /* throw */
                                        }
                                        catch (Exception e)
                                        {
                                            TraceOps.DebugTrace(
                                        ...
                                        if (FlagOps.HasFlags(
                                                pluginFlags, PluginFlags.System, true) != system)
                                        {
                                            continue;
                                        }
                                        ...
                                        aboutCode = GetPluginAbout(
                                            interpreter, plugin, ref aboutResult);

                                        if (aboutCode == ReturnCode.Ok)
                                        {
                                            WritePluginAbout(
                                                interpreter, displayHost, aboutResult,
                                                positioning, system, showBanner,
                                                showLegalese, showSource,
                                                showUpdate, wrotePlugin,
                                                ref wrote, ref wroteSingleLine,
                                                ref wroteMultiLine, ref wroteBox);
                                        }
                                    }
            ...
            )
        {
            if (interpreter != null)
            {
                ...
                if (displayHost != null)
                {
                    ...
                    bool showGroups = true; /* TODO: Good default? */
                    bool showTopics = true; /* TODO: Good default? */
                    bool showHeader = true; /* TODO: Good default? */

                    if ((arguments != null) && (arguments.Count >= 3))
                    {
                        if (Value.GetBoolean2(arguments[2],
                                ValueFlags.AnyBoolean, interpreter.CultureInfo,
                    ...
                    {
                        if (Value.GetBoolean2(arguments[3],
                                ValueFlags.AnyBoolean, interpreter.CultureInfo,
                    ...
                    {
                        if (Value.GetBoolean2(arguments[4],
                                ValueFlags.AnyBoolean, interpreter.CultureInfo,
                    ...
                    StringListDictionary groups = showGroups ?
                        GetCachedInteractiveCommandGroups(interpreter) : null;

                    StringPairDictionary help = showTopics ?
                        GetCachedInteractiveCommandHelp(interpreter) : null;
                    ...
                    if (topic != null)
                    {
                        ...
                        HostTryWriteColor(displayHost,
                            FormatHelpItem(interpreter, groups, help, topic, false),
                            true, helpItemForegroundColor, helpItemBackgroundColor);
                    ...
                    else
                    {
                        ...
                        if (showHeader)
                        {
                            ...
                            HostTryWriteColor(displayHost, String.Format(
                                ...
                                FormatOps.WrapOrNull(prefix), prefix), true, helpForegroundColor, helpBackgroundColor);

                            displayHost.WriteLine();

                            HostTryWriteColor(displayHost, String.Format(
                                "Please type \"{0}help ?topic?\" for more information on a specific topic.",
                                prefix), true, helpForegroundColor, helpBackgroundColor);
                        }
                        ...
                        if (help != null)
                        {
                            ...
                            if (groups != null)
                            {
                                ...
                                if ((keys != null) && (keys.Count > 0))
                                {
                                    foreach (string key in keys.Keys)
                                    {
                                        ...
                                        HostTryWriteColor(displayHost, String.Format(
                                            "The {0} available {1} {2}: ",
                                            topics.Count, FormatOps.WrapOrNull(key),
                                            topics.Count != 1 ? "commands are" : "command is"),
                            ...
                            else
                            {
                                ...
                                if ((keys != null) && (keys.Count > 0))
                                {
                                    ...
                                    HostTryWriteColor(displayHost, String.Format(
                                        "The {0} available {1}: ", keys.Count,
                                        keys.Count != 1 ? "commands are" : "command is"),
                                        true, helpForegroundColor, helpBackgroundColor);
                        ...
                        else if (groups != null)
                        {
                            ...
                            if ((keys != null) && (keys.Count > 0))
                            {
                                ...
                                HostTryWriteColor(displayHost, String.Format(
                                    "The {0} available command {1}: ", keys.Count,
                                    keys.Count != 1 ? "groups are" : "group is"),
                                    true, helpForegroundColor, helpBackgroundColor);
