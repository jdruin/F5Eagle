...
namespace Eagle._Components.Private.Tcl
{
    ...
    static class TclWrapper
    {
        ...
        //
        // NOTE: This is the bare minimum version of Tcl/Tk that we support and
        ...
        //
        // NOTE: The shared library extension pattern fragments for the
        //       platforms we support.
        //
        private static readonly string windowsLibraryExtensionPattern = "\\.dll";
        private static readonly string unixLibraryExtensionPattern = "\\.(?:dll|sl|so)";
        private static readonly string macintoshLibraryExtensionPattern = "\\.dylib";
        #endregion

        ///////////////////////////////////////////////////////////////////////////////////////////////

        //
        // NOTE: The file name pattern(s) for finding candidate Tcl and/or Tk
        //       dynamic link library file names that we might want to load.
        //
        ...
        //
        private static readonly Regex windowsLibraryRegEx = new Regex(
            "^(tcl\\d+[t]?[s]?[g]?[x]?" +
            windowsLibraryExtensionPattern + ")$",
            RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);

        private static readonly Regex windowsBaseKitRegEx = new Regex(
            "^(base-(?:tcl|tk)\\d+\\.\\d+-thread-win32-\\w+" +
            windowsLibraryExtensionPattern + ")$",
            RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);

        private static readonly Regex windowsTclKitRegEx = new Regex(
        ...
        //
        private static readonly Regex unixLibraryRegEx = new Regex(
            "^(libtcl\\d+\\.?\\d+" + unixLibraryExtensionPattern +
            "(?:\\.\\d+\\.\\d+)?)$",
            RegexOptions.CultureInvariant);

        private static readonly Regex unixBaseKitRegEx = new Regex(
            "^(base-(?:tcl|tk)\\d+\\.\\d+-thread-\\w+-\\w+" +
            unixLibraryExtensionPattern + "(?:\\.\\d+\\.\\d+)?)$",
            RegexOptions.CultureInvariant);

        private static readonly Regex unixTclKitRegEx = new Regex(
            "^(libtclkit\\d\\.?\\d(?:\\.?\\d{1,2})?(?:(?:a|b)(?:\\d+)?)?" +
            unixLibraryExtensionPattern + "(?:\\.\\d+\\.\\d+)?)$",
            RegexOptions.CultureInvariant);
        #endregion
        ...
        //
        private static readonly Regex macintoshLibraryRegEx = new Regex(
            "^(libtcl\\d+\\.?\\d+" +
            macintoshLibraryExtensionPattern + ")$",
            RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);

        private static readonly Regex macintoshBaseKitRegEx = new Regex(
            "^(base-(?:tcl|tk)\\d+\\.\\d+-thread-macosx-\\w+" +
            macintoshLibraryExtensionPattern + ")$",
            RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);

        private static readonly Regex macintoshTclKitRegEx = new Regex(
            "^(libtclkit\\d\\.?\\d(?:\\.?\\d{1,2})?(?:(?:a|b)(?:\\d+)?)?" +
            macintoshLibraryExtensionPattern + ")$",
            RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
        #endregion
        ...
        #region Master File Name Pattern Lists
        //
        // NOTE: Create the list of primary regular expression patterns to
        //       check candidate file names against.
        //
        private static readonly RegExList primaryNameRegExList = new RegExList(new Regex[] {
            windowsLibraryRegEx,
            unixLibraryRegEx,
        ...
        //
        // NOTE: Create the list of secondary regular expression patterns to
        //       check candidate file names against.
        //
        private static readonly RegExList secondaryNameRegExList = new RegExList(new Regex[] {
            windowsTclKitRegEx,
            unixTclKitRegEx,
        ...
        //
        // NOTE: Create the list of "other" regular expression patterns to
        //       check candidate file names against.
        //
        private static readonly RegExList otherNameRegExList = new RegExList(new Regex[] {
            windowsBaseKitRegEx,
            unixBaseKitRegEx,
        ...
        //
        // NOTE: The file name pattern(s) for extracting the Tcl version number
        //       (and possibly the threaded flag) from the file name.
        //
        ...
        //
        private static readonly Regex windowsLibraryVersionRegEx = new Regex(
            "^tcl(\\d+)([t])?[s]?([g])?[x]?" +
            windowsLibraryExtensionPattern + "$",
            RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);

        private static readonly Regex windowsBaseKitVersionRegEx = new Regex(
            "^base-(?:tcl|tk)(\\d+\\.\\d+)(-thread)?-win32-\\w+" +
            windowsLibraryExtensionPattern + "$",
            RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);

        private static readonly Regex windowsTclKitVersionRegEx = new Regex(
            "^libtclkit(\\d\\.?\\d(?:\\.?\\d{1,2})?)(?:(?:a|b)(?:\\d+)?)?" +
            windowsLibraryExtensionPattern + "$",
            RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
        #endregion
        ...
        //
        private static readonly Regex unixLibraryVersionRegEx = new Regex(
            "^libtcl(\\d+\\.?\\d+)" +
            unixLibraryExtensionPattern + "(?:\\.\\d+\\.\\d+)?$",
            RegexOptions.CultureInvariant);

        private static readonly Regex unixBaseKitVersionRegEx = new Regex(
            "^base-(?:tcl|tk)(\\d+\\.\\d+)(-thread)?-\\w+-\\w+" +
            unixLibraryExtensionPattern + "(?:\\.\\d+\\.\\d+)?$",
            RegexOptions.CultureInvariant);

        private static readonly Regex unixTclKitVersionRegEx = new Regex(
            "^libtclkit(\\d\\.?\\d(?:\\.?\\d{1,2})?)(?:(?:a|b)(?:\\d+)?)?" +
            unixLibraryExtensionPattern + "(?:\\.\\d+\\.\\d+)?$",
            RegexOptions.CultureInvariant);
        #endregion
        ...
        //
        private static readonly Regex macintoshLibraryVersionRegEx = new Regex(
            "^libtcl(\\d+\\.?\\d+)" +
            macintoshLibraryExtensionPattern + "$",
            RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);

        private static readonly Regex macintoshBaseKitVersionRegEx = new Regex(
            "^base-(?:tcl|tk)(\\d+\\.\\d+)(-thread)?-macosx-\\w+" +
            macintoshLibraryExtensionPattern + "$",
            RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);

        private static readonly Regex macintoshTclKitVersionRegEx = new Regex(
            "^libtclkit(\\d\\.?\\d(?:\\.?\\d{1,2})?)(?:(?:a|b)(?:\\d+)?)?" +
            macintoshLibraryExtensionPattern + "$",
            RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
        #endregion
        ...
        #region Master File Name Pattern Dictionary (OperatingSystemId)
        //
        // NOTE: Create a dictionary that maps regular expression patterns to a
        //       particular operating system we support.
        //
        private static readonly RegExEnumDictionary primaryVersionRegExDictionary =
        ...
        //
        // NOTE: Used to delimit the major, minor, build, and revision numbers
        //       in a version string.
        //
        ...
        //       to be able to yank the entire native Tcl library out
        ...
        public static IntPtr NewByteArray( /* NOT USED */
            ITclApi tclApi, /* in */
            byte[] bytes    /* in */
            )
        {
            ...
            )
        {
            ...
            code = FileOps.GetFileVersion(fileName, ref version, ref error);
        ...
        public static ReturnCode SetSlaveCancelFlags(
            ITclApi tclApi,      /* in */
            IntPtr interp,       /* in */
            Tcl_EvalFlags flags, /* in */
            bool force,          /* in */
            ...
            )
        {
            try
            {
                if (TclApi.CheckModule(tclApi, ref error))
                {
                    TclSetSlaveCancelFlags setSlaveCancelFlags;

                    lock (tclApi.SyncRoot)
                    {
                        setSlaveCancelFlags = tclApi.SetSlaveCancelFlags;
                    }

                    if (tclApi.CheckInterp(interp, ref error))
                    {
                        if (setSlaveCancelFlags != null)
                        {
                            /* NO RESULT */
                            setSlaveCancelFlags(interp, flags, ConversionOps.ToInt(force));
            ...
            )
        {
            ...
            return DoOneEvent(
                interpreter, timeout, wait, all, noComplain, ref count, ref tclApi, ref error);
        }
                                ...
                                try
                                {
                                    ...
                                    if (all)
                                    {
                                            ...
                                            ((newCount = doOneEvent(flags)) != 0))
                                        {
                                            ...
                                            //
                                            // NOTE: Yield to other running threads.  This also gives them an
                                            //       opportunity to cancel the script in progress on this thread.
                                            //
                                            EventOps.Sleep(interpreter, false);
                                        }
                                    }
                                    else
                                    {
                ...
                try
                {
                    ...
                    if (!String.IsNullOrEmpty(fileName))
                    {
...
#endif
                        {
                            ...
                            Tcl_ReleaseLevel releaseLevel = (fileVersion != null) ?
                                (Tcl_ReleaseLevel)fileVersion.Build :
                                Tcl_ReleaseLevel.TCL_UNKNOWN_RELEASE;
            ...
            )
        {
            ...
            result = PathOps.CombinePath(
                null, result, TclVars.PackageName, TclVars.LibPath);
            ...
            )
        {
            ...
            result = PathOps.CombinePath(
                null, result, TclVars.PackageName, TclVars.BinPath);
            ...
            )
        {
            ...
            //
            if (!FlagOps.HasFlags(flags, FindFlags.RecursivePaths, true))
            ...
            try
            {
                ...
                foreach (StringPair pair in PathOps.GetPathList(list))
                {
                    ...
                    if (File.Exists(path))
                    {
                        ...
                        if (LooksLikeTclLibrary(
                                path, allFlags, ref priority, ref errors))
                        {
                            TclBuild build = null;
                    ...
                    {
                        string[] fileNames = GetFileNames(path, allFlags);
                            ...
                            foreach (string subKeyName in key.GetSubKeyNames())
                            {
                                        ...
                                        subKeyName))
                                {
                                    if (subKey == null)
                                    {
                                        ...
                                        {
                                            MaybeAddAnError(ref errors, String.Format(
                                                "could not open sub-key \"{0}\" of " +
                                                "registry key \"{1}\"", subKeyName,
                                                key));
                                        }
            ...
            )
        {
            ...
            //
            //       15. The argument "callback" may provide a delegate to be
        ...
        retry:
            ...
            if (FlagOps.HasFlags(flags, FindFlags.SpecificPath, true))
            {
                if (paths != null)
                {
                    ...
                    {
                        if (FindViaPath(
                                FindFlags.SpecificPath, flags, path, unknown, clientData,
            ...
            {
                StringList autoPaths = GlobalState.GetAutoPathList(
                    interpreter, FlagOps.HasFlags(flags,
                    FindFlags.RefreshAutoPath, true));

                if (autoPaths != null)
                {
                    foreach (string autoPath in autoPaths)
                    {
                        ...
                        if (FindViaPath(
                                ...
                                autoPath, unknown, clientData, ref builds,
                                ref errors) != ReturnCode.Ok)
                        {
                            MaybeAddAnError(ref errors, String.Format(
                                "find Tcl library builds via auto-path \"{0}\" failed",
                                FormatOps.DisplayName(autoPath)));
                        }
                    }
                }
                else
            ...
            if (FlagOps.HasFlags(flags, FindFlags.PackageBinaryPath, true))
            {
                if (FlagOps.HasFlags(flags, FindFlags.FindArchitecture, true) &&
                    FindViaPath(
                        ...
                        TclVars.BinPath)), unknown, clientData, ref builds,
                        ref errors) != ReturnCode.Ok)
                {
                    MaybeAddAnError(ref errors,
            ...
            if (FlagOps.HasFlags(flags, FindFlags.PackageLibraryPath, true))
            {
                if (FlagOps.HasFlags(flags, FindFlags.FindArchitecture, true) &&
                    FindViaPath(
                        ...
                        GetAlternateProcessorPath(PathOps.CombinePath(
                            null, GlobalState.GetTclPackageNamePath(),
                        TclVars.LibPath)), unknown, clientData, ref builds,
                        ref errors) != ReturnCode.Ok)
                {
                    MaybeAddAnError(ref errors,
            ...
            if (FlagOps.HasFlags(flags, FindFlags.BinaryPath, true))
            {
                if (FindViaPath(
                        ...
                        GlobalState.GetBinaryPath(), unknown, clientData,
                        ref builds, ref errors) != ReturnCode.Ok)
                {
                    MaybeAddAnError(ref errors,
            ...
            {
                if (FindViaPath(
                        FindFlags.LocalLibraryPath, flags, String.Format(
                        "{0}{1}", GetProcessorPath(TclVars.UserLocalLibPath),
                        TclVars.LinuxGnuSuffix), unknown, clientData,
                        ref builds, ref errors) != ReturnCode.Ok)
                {
                    MaybeAddAnError(ref errors,
                ...
                if (FindViaPath(
                        ...
                        TclVars.UserLocalLibPath, unknown, clientData,
                        ref builds, ref errors) != ReturnCode.Ok)
                {
                    MaybeAddAnError(ref errors,
            ...
            {
                if (FindViaPath(
                        FindFlags.LibraryPath, flags, String.Format(
                        "{0}{1}", GetProcessorPath(TclVars.UserLibPath),
                        TclVars.LinuxGnuSuffix), unknown, clientData,
                        ref builds, ref errors) != ReturnCode.Ok)
                {
                    MaybeAddAnError(ref errors,
                ...
                if (FindViaPath(
                        FindFlags.LibraryPath, flags, TclVars.UserLibPath,
                        unknown, clientData, ref builds, ref errors) != ReturnCode.Ok)
                {
                    MaybeAddAnError(ref errors,
        ...
        filter:
            ...
            if (builds != null)
            {
                ...
                {
                    StringList keys = new StringList(builds.Keys);

                    foreach (string key in keys)
                    {
                        //
                        // NOTE: Get the build object associated with this key
                        //       (file name).
                        //
                        ...
                        {
                            //
                            // NOTE: Compare the version of the current build with
                            //       the minimum and/or maximum required versions.
                            //       If the current build does not meet the criteria,
                            //       remove it from the resulting dictionary.
                            //
                            if (((minimumRequired == null) ||
                                    PackageOps.VersionCompare(build.PatchLevel,
                                        minimumRequired) >= 0) &&
                            ...
                            {
                                //
                                // NOTE: This build is ok, skip removing it.
                                //
                                continue;
                            }
                        }
            ...
            )
        {
            ...
            //
            //       2. If the "architecture" flag is set, we refuse to consider any
            //          library that does not match the architecture for the current
            //          process.
            //
            //       3. We refuse to consider any library that does not meet the
            ...
            //          is waived).
            //
            //       5. We always favor libraries with a higher release level.  This
            //          allows us to prefer final releases over alphas and betas of
            //          the same base version.
            //
            //       6. We always favor libraries that are "trusted" over ones that
            //          are not (i.e. those with Authenticode signatures).
            //
            //       7. We always favor libraries with a higher relative priority
            ...
            //
            //       8. We always favor threaded libraries (if the file name contains
            //          this information).
            //
            //       9. We always favor the DEBUG libraries if we are also compiled
            //          in DEBUG.
            //
            //      10. We always validate that we are loading a threaded build
            ...
            //
            //      11. We always favor libraries with a lower relative sequence
            ...
            try
            {
                if ((builds != null) && (builds.Count > 0))
                {
                    ...
                    foreach (KeyValuePair<string, TclBuild> pair in builds)
                    {
                        ...
                        if (String.IsNullOrEmpty(thisFileName) || !File.Exists(thisFileName))
                            continue;

                        //
                        // NOTE: Skip over invalid entries.
                        //
                            ...
                            (thisBuild.Magic != processMagic))
                        {
                            MaybeAddAnError(ref errors, String.Format(
                                ...
                                FormatOps.DisplayName(thisFileName), thisBuild.Magic,
                                processMagic));
                                ...
                                thisBuild.PatchLevel, minimumRequired) < 0)
                        {
                            MaybeAddAnError(ref errors, String.Format(
                                ...
                                FormatOps.DisplayName(thisFileName), thisBuild.PatchLevel,
                                minimumRequired));
                        ...
                        bool thisThreaded = thisBuild.Threaded || thisBuild.DefaultThreaded;
                        ...
                        if (bestDebug && (patchLevelResult <= 0) && (priorityResult <= 0))
                            continue;
                ...
                if (tclApi == null)
                {
                    if (interp == IntPtr.Zero)
                    {
                        ...
                        code = Find(
                            ...
                            clientData, ref builds, ref errors);
                                                    ...
                                                    (PackageOps.VersionCompare(loaded, minimumRequired) >= 0))
                                                {
                                                        ...
                                                        (PackageOps.VersionCompare(loaded, maximumRequired) <= 0))
                                                    {
                                                        ...
                                                        if (code == ReturnCode.Ok)
                                                        {
                                                            ...
                                                            if (createInterp != null)
                                                                //
                                                                // NOTE: Attempt to create a master interp.
                                                                //
                                                                newInterp = createInterp();
                                                            ...
                                                            if (newInterp != IntPtr.Zero)
                                                            {
                                                                ...
                                                                if (!GetInterpActive(tclApi, newInterp))
                                                                {
                                                                    ...
                                                                    //       Tcl scripts.
                                                                    //
                                                                    patchLevelNameObjPtr = NewString(tclApi, TclVars.PatchLevelName);
...
#endif
                                                                        {
...
#endif
                                                                            {
                                                                                ...
                                                                                if (code == ReturnCode.Ok)
                                                                                {
                                                                                    ...
                                                                                    {
                                                                                        //
                                                                                        // NOTE: Return the newly created master Tcl interpreter
                                                                                        //       to the caller.
                                                                                        //
                                                                                        interp = newInterp;
                ...
                if (interpreter != null)
                {
                    if (TclApi.CheckModule(tclApi, ref error))
                    {
                        ...
                        if (noThread || (interp == IntPtr.Zero) || tclApi.CheckInterp(interp, ref error))
                        {
                            ...
                            //
                            // NOTE: If the master Tcl interpreter needs to be deleted, do it now.
                            //       The master Tcl interpreter is a per-interpreter resource, not
                            ...
                            //
                            // NOTE: Make 100% sure that we were able to delete the master Tcl
