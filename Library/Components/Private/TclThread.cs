    ...
    sealed class TclThread : ISynchronize, IDisposable
    {
            ...
            )
        {
            ...
            thread = Engine.CreateThread(
                interpreter, ThreadStart, 0, true, false);
            ...
            )
        {
            ...
            PrivateTryLock(milliseconds, ref locked);
        }
            ...
            ) /* throw */
        {
            ...
            //
            // BUGFIX: These must be grabbed prior to grabbing and holding
                ...
                if (!disposed)
                {
                    if (!this.disposing)
                    {
                        ...
                        try
                        {
                            ...
                            //
                            // NOTE: Attempt to shutdown the Tcl interpreter
                            //       thread now (if it is still alive).
                            //
                            if (!finalized)
                            {
                                ...
                                shutdownCode = Shutdown(
                                    tclApi, tclBridges, true, true, false,
                                    ref shutdownError);

                                //
                                // NOTE: If we failed to shutdown the thread,
                                //       complain loudly.
                                //
                                if (shutdownCode != ReturnCode.Ok)
                                {
                                    DebugOps.Complain(
                                        interpreter, shutdownCode,
                                        shutdownError);
                                }
                            }
            ...
            )
        {
            ...
            if (!IsNotifierUsable(thread, ref usableError))
            {
                if (debug)
                {
                    int threadId = GlobalState.GetCurrentNativeThreadId();
                            ...
                            ref startEvent, ref doneEvent, ref idleEvent, ref queueEvent))
                    {
                        ...
                        while (true)
                        {
                            ...
                            else
                            {
                                ...
                                //
                                if (!DoOneEvent(this, interpreter, timeout, debug, true))
                                    done = true;
                            ...
                            //
                            // NOTE: Finally, yield to other running threads.
                            //
                            HostOps.Yield();
                        }
                        ...
                        if (!ThreadOps.WasAnyWaitFailed(index))
                        {
                            ...
                            //       called method if necessary.
                            //
                            /* IGNORED */
                            DoOneEvent(this, interpreter, timeout, debug, true);
                        }
                    }
                }
                catch (Exception e)
                    ...
                    if (queueEvent != null)
                    {
                        ...
                        try
                        {
                            ...
                            @event = Event.Create(
                                ...
                                null, new ClientData(data), ref error);
            ...
            )
        {
            ...
            if (interpreter != null)
            {
                ...
                finally
                {
                    ...
                    if (NativeOps.IsValidHandle(thread))
                    {
                        NativeOps.UnsafeNativeMethods.CloseHandle(thread);
                        thread = IntPtr.Zero;
                    }
                }
            }
                                ...
                                if (@event != null)
                                {
                                    ...
                                    if (eventDebug)
                                    {
                                        TraceOps.DebugTrace(threadId, String.Format(
                                            ...
                                            FormatOps.WrapOrNull(@event.Name), TimeOps.GetUtcNow(),
                                            GlobalState.GetCurrentNativeThreadId(), FormatOps.WrapOrNull(
                                            @event.ToList())), typeof(Tcl_EventCallback).Name,
                                            TracePriority.EventDebug);
                                    }
                                    ...
                                    if (code == ReturnCode.Ok)
                                    {
                                            ...
                                            case EventType.AddCommand:
                                                {
                                                    if (eventClientData != null)
                                                    {
                                                        IAnyTriplet<string, ICommand, bool> anyTriplet =
                                                            eventClientData.Data as
                                                            IAnyTriplet<string, ICommand, bool>;
                                        ...
                                        if (eventDebug)
                                        {
                                            TraceOps.DebugTrace(threadId, String.Format(
                                                ...
                                                "code = {4}, result = {5}",
                                                FormatOps.WrapOrNull(@event.Name), TimeOps.GetUtcNow(),
                                                FormatOps.Performance(performanceClientData),
