...
namespace Eagle._Components.Public
{
        ...
        ICloneable, IParseState
    {
        ...
        public ParseState(
            ...
            : this(engineFlags, substitutionFlags, null, currentLine)
        {
            // do nothing.
        }
        ...
        public virtual object Clone()
        {
            IParseState parseState = new ParseState(this);

            //
            // NOTE: Make a deep copy of the token list itself; however, they
            //       will share the same IToken objects.  There is no nice way
            //       to avoid this and produce a truly deep copy because we do
            //       not know the precise type of each contained IToken object.
            //
            parseState.Tokens = new TokenList(this.Tokens);
    ...
    public static class Parser
    {
            ...
            )
        {
            ...
            if (parseState != null)
            {
                ...
                if (Engine.HasNoCancel(engineFlags))
                    readyFlags |= ReadyFlags.NoCancel;

#if DEBUGGER
                if (Engine.HasNoBreakpoint(engineFlags))
            ...
            )
        {
            if (!String.IsNullOrEmpty(text))
            {
                ...
                for (int index = 0; index < length; index++)
                {
                    ...
                    if (character == Characters.OpenParenthesis)
                    {
                        char lastCharacter = text[length - 1];

                        if (lastCharacter == Characters.CloseParenthesis)
                        {
                            error = notScalarError;
                            return false;
                        }
                    }
                    else if ((character == Characters.Colon) && ((index + 1) < length))
                    {
                        char nextCharacter = text[index + 1];

                        if (nextCharacter == Characters.Colon)
                        {
        ...
        internal static ReturnCode SplitVariableName(
            string name,
            ref string varName,
            ref string varIndex,
            ref Result error
            )
        {
            ...
            int openParenthesis = Index.Invalid;
            int closeParenthesis = Index.Invalid;

            for (int index = 0; index < length; index++)
            {
                if (name[index] == Characters.OpenParenthesis)
                {
                    if ((length > 1) &&
                        (name[length - 1] == Characters.CloseParenthesis))
                    {
                        openParenthesis = index;
                        closeParenthesis = length - 1;
                    }
                    break;
                }
            }
        ...
        private static ReturnCode FindElement(
            /* Interpreter interpreter, */ /* NOT USED */
            string text,
            int startIndex,
            int length,
        ...
        internal static ReturnCode SplitList(
            ...
            )
        {
            ...
            return SplitList(
                interpreter, text, startIndex, length,
        ...
        {
            if (UseNativeSplitList && (startIndex == 0) && (length <= 0))
            {
                bool locked = false;
                ...
                try
                {
                    ...
                    if (locked && NativeUtility.IsAvailable(interpreter))
                    {
                        ...
                        ReturnCode code = NativeUtility.SplitList(
                            text, ref localList, ref error);
                    ...
                    {
                        TraceOps.DebugTrace(
                            "SplitList: unable to acquire native utility lock",
                            typeof(Parser).Name, TracePriority.LockError);
                    }
                }
                finally
                {
