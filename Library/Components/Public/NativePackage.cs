...
namespace Eagle._Components.Public
{
    ...
    public static class NativePackage
    {
        ...
        //       the constants tclMasterInterpPrefix, tclSafeInterpPrefix, and
        //       tclInterpPrefix in the Interpreter class.
        //
        private const string tclNativeMasterInterpPrefix = "nativeMasterInterp";
        private const string tclNativeSafeMasterInterpPrefix = "nativeSafeMasterInterp";
            ...
            )
        {
            if (master)
            {
                return safe ? tclNativeSafeMasterInterpPrefix :
                    tclNativeMasterInterpPrefix;
            }
            else
            {
            ...
            )
        {
                    ...
                    tclInterps) /* O(N) */
            {
                ...
                if (GetInterpreter(
                        ...
                        ref childInterpreter) == ReturnCode.Ok)
                {
                    DebugWriteOrTrace(interpreter, String.Format(
                        ...
                        FormatOps.InterpreterNoThrow(childInterpreter),
            ...
            try
            {
                ...
                if (locked)
                {
                        ...
                        tclInterps.ContainsValue(interp) /* O(N) */)
                    {
                        ...
                        if (interpreter != null)
                        {
                            int localThreadId = interpreter.GetTclThreadId();
        ...
        //       class to help implement the [tcl master] sub-command.
        //
        internal static ReturnCode GetMasterTclInterpreter(
            Interpreter interpreter,
            ref string name,
            ref IntPtr interp,
            ref Result error
            ...
            lock (syncRoot) /* TRANSACTIONAL */
            {
                if (HasTclInterpreters(false, ref error))
                {
                    ...
                    string pattern = FormatOps.Id(
                        tclNativeMasterInterpPrefix, null,
                        Characters.Asterisk.ToString());

                    string safePattern = FormatOps.Id(
                        tclNativeSafeMasterInterpPrefix, null,
                        Characters.Asterisk.ToString());
                            ...
                            tclInterps) /* O(N) */
                    {
                        ...
                        //
                        // NOTE: Second, make sure it is a master (or "safe"
                        //       master) Tcl interpreter.  Hard-coded match
                        //       mode is OK here.
                        //
                        if ((StringOps.Match(
        ...
        {
            if (String.Equals(protocolId, ProtocolIdV1R0,
                    StringOps.SystemStringComparisonType) ||
                String.Equals(protocolId, ProtocolIdV1R1,
                    StringOps.SystemStringComparisonType))
            {
                haveExtra = false;
                return ReturnCode.Ok;
            ...
            if (String.Equals(protocolId, ProtocolIdV1R2,
                    StringOps.SystemStringComparisonType))
            {
                haveExtra = true;
                return ReturnCode.Ok;
                ...
                if (code == ReturnCode.Ok)
                {
                    ...
                    try
                    {
                        ...
                        if (interpreter == null)
                        {
                            ...
                            //       flags (i.e. ), calculate out the exact
