...
namespace Eagle._Forms
{
    ...
    internal partial class UpdateForm : Form
    {
            ...
            )
        {
            ...
            {
                value = (double)e.BytesReceived / e.TotalBytesToReceive;
            }
            ...
            new Thread(delegate()
            {
                try
                {
                    ...
                    QueueStatus(String.Format(
                        ...
                        "item(s), and {4} other item(s).", releases.Count,
                        ((protocolCounts != null) &&
                    ...
                    QueueStatus("Found release matching configuration.");

                    release.Dump();

                    if (!release.IsValid)
                    {
                        QueueStatus("Release is invalid.");
                        goto done;
                    }

                    QueueStatus("Release is valid.");

                    if (!release.IsGreater)
                    {
                        QueueStatus("Release is not newer.");
                        goto done;
                    }
                    ...
                    //
                    if (release.IsSelf)
                        QueueStatus("Release is for the updater itself.");
                    ...
                    string downloadDirectory = Path.Combine(Path.GetTempPath(),
                    ...
                    try
                    {
                        Directory.CreateDirectory(downloadDirectory); /* throw */

                        QueueStatus("Local download directory created.", true);
                    }
                    catch (Exception ex)
                    {
                        Trace(ex);

                        QueueStatus("Failed to create local download directory.");
                        goto done;
                    }
                    ...
                    EnableUpdateButton(false, true);
                    TraceAndUpdateStatus("Downloading release file...");
                    UpdateStatusUri(uri.ToString());
