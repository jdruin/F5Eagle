...
#include <stdio.h>	/* NOTE: For swprintf, va_list, etc. */
...
static int		DemandExecuteClrMethod(HANDLE hModule,
			    ...
			    Tcl_Obj *assemblyPathPtr, Tcl_Obj *typeNamePtr,
			    Tcl_Obj *methodNamePtr, Tcl_Obj *argumentPtr,
			    MethodFlags methodFlags, LPDWORD pReturnValue);
				 ...
				 * well? */
{
	...
	pTclStubs->tclSetSlaveCancelFlags = tclIntStubsPtr->tclSetSlaveCancelFlags;
    }
    ...
    ...)		/* Strings to append to the message, if any. */
{
	...
	OutputDebugStringW(Tcl_GetUnicode(objv[1])); /* NON-PORTABLE */
    }
    ...
    BOOL bStrict)	    /* Fail if already loaded and/or started? */
{
	...
	if (pClrRuntimeHost == NULL) {
	    ...
	    if (!bLoadable) {
		if (interp != NULL) {
		    Tcl_AppendResult(interp, "CLR version \"",
			clrVersion, "\" not loadable\n", NULL);
		}
    ...
    Tcl_Interp *interp)			/* Current Tcl interpreter. */
{
	...
	code = GetAndExecuteClrMethod(hTclModule, &uTclStubs, interp,
	    NULL, METHOD_TYPE_STARTUP | METHOD_VIA_LOAD);
    ...
    Tcl_Obj *CONST objv[])	/* The arguments. */
{
	...
	case OPT_CLREXECUTE: {
	    ...
	    code = DemandExecuteClrMethod(hTclModule, &uTclStubs, interp,
		objv[2], objv[3], objv[4], objv[5], METHOD_TYPE_DEMAND |
	...
	case OPT_CONTROL: {
	    ...
	    code = GetAndExecuteClrMethod(hTclModule, &uTclStubs, interp,
		(listPtr != NULL) ? Tcl_GetUnicode(listPtr) : NULL,
	...
	case OPT_DETACH: {
	    ...
	    code = GetAndExecuteClrMethod(hTclModule, &uTclStubs, interp, NULL,
		METHOD_TYPE_DETACH | METHOD_VIA_COMMAND);
	...
	case OPT_SHUTDOWN: {
	    ...
	    code = GetAndExecuteClrMethod(hTclModule, &uTclStubs, interp, NULL,
		METHOD_TYPE_SHUTDOWN | METHOD_VIA_COMMAND);
	...
	case OPT_STARTUP: {
	    ...
	    code = GetAndExecuteClrMethod(hTclModule, &uTclStubs, interp, NULL,
		METHOD_TYPE_STARTUP | METHOD_VIA_COMMAND);
