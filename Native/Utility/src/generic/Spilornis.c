...
#include <malloc.h>		/* NOTE: For _msize, etc. */
#endif
 ...
 * decimal digit, or a hexadecimal digit.
 */
...
static LPCWSTR EaglePrintf(SIZE_T size, LPCWSTR format, ...);
...
static INT EagleTracePrintf(
    LPCSTR format,		/* The "printf-style" format string. */
    ...)			/* The extra arguments, if any. */
{
    va_list argList;
    CHAR buffer[LIBRARY_TRACE_BUFFER_SIZE + 1] = {0};
    INT result;

    va_start(argList, format);
    result = vsnprintf(buffer,
	LIBRARY_TRACE_BUFFER_SIZE, format, argList);
    va_end(argList);
			 ...
			 * string, if any. */
{
    ...
    if (length == 0) length = LIBRARY_RESULT_SIZE;
	...
	switch (*p) {
		...
		if (openBraces > 1) {
			...
			*errorPtr = EaglePrintf(0,
				L"list element in braces followed by \"%.*s\" %s",
				(int)(p2 - p), p, L"instead of space");
		    }
		    return EAGLE_ERROR;
		}
		break;
		...
		if (inQuotes) {
			...
			*errorPtr = EaglePrintf(0,
				L"list element in quotes followed by \"%.*s\" %s",
				(int)(p2 - p), p, L"instead of space");
		    }
		    return EAGLE_ERROR;
		}
...
{
    LPWSTR pBuffer = Eagle_AllocateMemory(LIBRARY_VERSION_SIZE);

    if (pBuffer == NULL) {
	return NULL;
    }

    swprintf(pBuffer, LIBRARY_VERSION_SIZE, L"%s v%s [%s %s]%s%s%s\0",
	...
	L" USE_32BIT_SIZE_T=" UNICODE_TEXT(STRINGIFY(USE_32BIT_SIZE_T)),
#else
	L"",
#endif
#if defined(USE_SYSSTRINGLEN)
	L" USE_SYSSTRINGLEN=" UNICODE_TEXT(STRINGIFY(USE_SYSSTRINGLEN))
#else
	L""
#endif
    );
...
#if defined(_MSC_VER) /* IMPLIED: !defined(NDEBUG) */
    LIBRARY_DEBUG(("Eagle_AllocateMemory: 0x%p, requested %Iu bytes, "
	"received %u bytes\n", pMemory, size, (pMemory != NULL) ?
	_msize(pMemory) : 0));
#endif
...
{
#if defined(_MSC_VER) /* IMPLIED: !defined(NDEBUG) */
    LIBRARY_DEBUG(("Eagle_FreeMemory: 0x%p, received %Iu bytes\n", pMemory,
	(pMemory != NULL) ? _msize(pMemory) : 0));
#endif
...
#if defined(_MSC_VER) && !defined(NDEBUG)
    memset(pMemory, LIBRARY_FREED_MEMORY, _msize(pMemory));
#endif
    ...
    LPCWSTR *ppError)		/* The error message, if any. */
{
    ...
    listLength = length;
    argc = Eagle_AllocateMemory(size * sizeof(SIZE_T));
    if (argc == NULL) {
	if (ppError != NULL) {
	    *ppError = EaglePrintf(0,
		L"out of memory for list element lengths");
	}
	return EAGLE_ERROR;
    }
    argv = Eagle_AllocateMemory(
	(size * sizeof(LPWSTR)) + ((listLength + 1) * sizeof(WCHAR)));
    if (argv == NULL) {
	if (ppError != NULL) {
	    *ppError = EaglePrintf(0,
		L"out of memory for list element pointers");
	}
    ...
    LPCWSTR *ppError)		/* The error message, if any. */
{
	...
	assert(elementCount > 0);
	flagPtr = Eagle_AllocateMemory(elementCount * sizeof(FLAGS));
	if (flagPtr == NULL) {
	    if (ppError != NULL) {
		*ppError = EaglePrintf(0,
		    L"out of memory for list element flags");
	    }
	    return EAGLE_ERROR;
	}
    }
    ...
    result = Eagle_AllocateMemory((numChars + 1) * sizeof(WCHAR));
    if (result == NULL) {
	if (ppError != NULL) {
	    *ppError = EaglePrintf(0,
		L"out of memory for list element text");
	}
